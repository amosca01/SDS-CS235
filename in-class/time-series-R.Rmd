...
Just be careful! A trend line is a **statistical transformation of the data**. It's up to you to determine is a particular statistical transformation is meaningful / appropriate!
...
Sometimes it can be useful to compare how multiple measures vary with respect to time, and to each other. Let's take a look at some data from [GapMinder](https://www.gapminder.org/), which the Stanford Data Challenge Lab has kindly compiled for us in the `dcldata` package:

```{r}
# devtools::install_github("dcl-docs/dcldata")
library(dcldata)
help(gm_life_expectancy)
```
...
Interestingly, all four countries experienced similar declines in the early 1990s. This suggests that South Africa’s decline in life expectancy might not be directly related to the end of apartheid. Want to explore further? Check out the rest of the `GapMinder` data available in the `dcldata` package to see what you can find!
